...
class RepresentativeViewHolder(val binding: ItemRepresentativeBinding) :
    ...
    fun bind(item: Representative) {
        ...
        binding.imAvatar.setImageResource(R.drawable.ic_profile)
    ...
    private fun showSocialLinks(channels: List<Channel>) {
        ...
        if (!facebookUrl.isNullOrBlank()) {
            enableLink(binding.icFacebook, facebookUrl)
        }

        val twitterUrl = getTwitterUrl(channels)
        if (!twitterUrl.isNullOrBlank()) {
            enableLink(binding.icTwitter, twitterUrl)
        }
    }

    private fun showWWWLinks(urls: List<String>) {
        if (urls.isEmpty()) return
        enableLink(binding.icWww, urls.first())
    }
